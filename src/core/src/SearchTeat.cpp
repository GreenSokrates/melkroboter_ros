...
void SearchTeat::initializeSubscribers()
{
	...
	sub_ = nh_.subscribe("/cloud/filteredAndRotated", 1,
...
void SearchTeat::initializePublishers()
{
	...
	pub_point_1 =
		nh_.advertise<geometry_msgs::PointStamped>("/melkroboter/teats/1", 1);
	pub_point_2 =
		nh_.advertise<geometry_msgs::PointStamped>("/melkroboter/teats/2", 1);
	pub_point_3 =
		nh_.advertise<geometry_msgs::PointStamped>("/melkroboter/teats/3", 1);
	pub_point_4 =
		nh_.advertise<geometry_msgs::PointStamped>("/melkroboter/teats/4", 1);
}
...
bool SearchTeat::Searchloop()
{
	...
	// coordinates
	// of the tip get pushed back into the vectors for the new teats and the
	// teatCount gets incremented
	for (size_t i = 0; i < teatCandidates.size(); i++)
	{
		if (isTeat(teatCandidates[i], cloud_freezed))
		{
			teatCount_new++;
			xVector_new.push_back(
				-(cloud_freezed->points[teatCandidates[i]].x));
			yVector_new.push_back(
				-(cloud_freezed->points[teatCandidates[i]].y));
...
void SearchTeat::publishPoint(geometry_msgs::PointStamped pointStamped,
							  int count)
{
	ROS_INFO("Got : %i", count);
	if (count == 1)
	{
		pub_point_1.publish(pointStamped);
	}
	if (count == 2)
	{
		pub_point_2.publish(pointStamped);
	}
	if (count == 3)
	{
		pub_point_3.publish(pointStamped);
	}
	if (count == 4)
	{
		pub_point_4.publish(pointStamped);
	}
}
...
void SearchTeat::updateCloud()
{
	...
	for (size_t i = 0; i < teatCandidates.size(); i++)
	{
		...
		viewer->addSphere(cloud->points[teatCandidates[i]], 0.002, 0, 255, 0,
						  candidateNr);
		viewer->spinOnce();
	}
}
